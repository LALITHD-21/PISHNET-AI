    ...
    >
      {(title || subtitle || headerAction) && (
        <div className="flex items-center justify-between mb-5 pb-4 border-b border-white/10 z-10 relative">
          <div>
            {title && (
              <h3 className="font-outfit text-3xl md:text-4xl font-black tracking-tighter text-[#EBEBEB] flex items-center gap-2">
                <span className="w-1.5 h-1.5 bg-primary inline-block rounded-full shadow-[0_0_12px_var(--primary)] animate-pulse"></span>
                {title}
              </h3>
            )}
            {subtitle && (
              <p className="cyber-metadata text-white/68 mt-2 leading-relaxed">{subtitle}</p>
            )}
          </div>
